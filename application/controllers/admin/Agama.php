...
class Agama extends CI_Controller {
    ...
    function index(){
        ...
        if(!empty($priv)){
            ...
            $config = array(
                'url'           => 'admin/agama/getdata',
                'variable'      => array('idx' => 'id_agama','cb'=>'agama'),
                'field'         => $field,
                'function'      => 'getData',
                'keyword_id'    => 'q',
                'param_id'      => array(),
            ...
            $data=array(
                'libjs'=>array('js/app/agama.js'),
                'ajaxdata' => getData($config),
                'content'=> $this->load->view('admin/agama_index','',true),
    ...
    function getdata()
    {
        ...
        if(!empty($priv)){
            ...
            $response = array(
                ...
                'message'   => "OK",
                'start'     => $mulai,
                'row_count' => $this->Agama_model->countData($q),
                'limit'     => $limit,
                'data'      => $this->Agama_model->getData($limit, $mulai, $q),
            );
        }else{
    ...
    function simpan(){
        ...
        if(!empty($priv)){
            ...
            $row=$this->Agama_model->getDataByid($id_agama);
            if(empty($row)){
                ...
                if($this->form_validation->run())
                {
                    ...
                    $insert = $this->Agama_model->simpanData($data);
                    header('Content-Type: application/json');
                ...
                if($this->form_validation->run())
                {
                    ...
                    $this->Agama_model->updateData($data,$id_agama);
                    header('Content-Type: application/json');
    ...
    function hapus($idx)
    {
        ...
        $priv=getAkses($link,$this->session->userdata('level'));
        if(!empty($priv)){
            $this->Agama_model->hapusData($idx);
            $response = array(
                'status'    => true,
