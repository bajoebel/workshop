...
class Liburnasional_model extends CI_Model
{
    ...
    function getData($limit=10,$mulai=0,$q){
        ...
        return $this->db->get('stx_libur a')->result();
    }
    function countData($q){
        ...
        $this->db->or_like('libur_keterangan',$q);
        $this->db->group_end();
        return $this->db->get('stx_libur a')->num_rows();
    }
    function getDataByid($libur_id){
        $this->db->where('libur_id',$libur_id);
        return $this->db->get('stx_libur')->row();
    }
    function simpanData($data){
        $this->db->insert('stx_libur',$data);
        return $this->db->insert_id();
    }
    function updateData($data,$libur_id){
        $this->db->where('libur_id',$libur_id);
        $this->db->update('stx_libur',$data);
    }
