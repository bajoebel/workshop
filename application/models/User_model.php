...
class User_model extends CI_Model
{
    ...
    function getData($limit=10,$mulai=0,$q){
        $this->db->join('stx_level b','a.user_idxlevel=b.idx');
        $this->db->group_start();
        ...
        return $this->db->get('stx_user a')->result();
    }
    function countData($q){
        ...
        $this->db->or_like('level',$q);
        $this->db->group_end();
        return $this->db->get('stx_user a')->num_rows();
    }
    ...
    function simpanData($data){
        $this->db->insert('stx_user',$data);
        return $this->db->insert_id();
    }
    function updateData($data,$user_id){
        $this->db->where('user_id',$user_id);
        $this->db->update('stx_user',$data);
    }
    function upload_files($path, $title, $files, $allow_types)
    {
        ...
        foreach ($files['name'] as $key => $image) {
            ...
            $config['file_name'] = $path .$fileName;
    ...
    function hapusData($user_id){
        $this->db->where('user_id',$user_id);
        $this->db->delete('stx_user');
    }
    function getLevel(){
        $this->db->where('aktif',1);
        return $this->db->get('stx_level')->result();
    }
}