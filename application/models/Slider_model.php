...
class Slider_model extends CI_Model
{
    ...
    function getData($limit=10,$mulai=0,$q=""){
        $this->db->group_start();
        $this->db->like('slider_caption',$q);
        $this->db->or_like('slider_status',$q);
        $this->db->or_like('slider_caption',$q);
        $this->db->group_end();
        $this->db->limit($limit, $mulai);
        $this->db->order_by('slider_id','desc');
        return $this->db->get('stx_slider a')->result();
    }
    function countData($q){
        ...
        $this->db->or_like('slider_caption',$q);
        $this->db->group_end();
        return $this->db->get('stx_slider a')->num_rows();
    }
    function getDataByid($slider_id){
        $this->db->where('slider_id',$slider_id);
        return $this->db->get('stx_slider')->row();
    }
    function simpanData($data){
        $this->db->insert('stx_slider',$data);
        return $this->db->insert_id();
    }
    function updateData($data,$slider_id){
        $this->db->where('slider_id',$slider_id);
        $this->db->update('stx_slider',$data);
    }
